...
Пакет для организации телеуправления роботизированной рукой **SOARM101**: ведомая рука (`follower`) повторяет движения ведущей (`leader`). Запускает обе руки, ретранслирует положения суставов и схвата, публикует статические трансформации и маркеры для RViz.
...
- **В метадокументации не описаны teleop-специфичные интерфейсы:**  
  В глобальном README перечислены только общие топики/действия. Для телеуправления полезно добавить:  
